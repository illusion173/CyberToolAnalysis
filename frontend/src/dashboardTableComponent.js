...
function ToolTable() {
  ...
  const handleToolFunction = (e) => {
    ...
    // All the options 
    const filterOptions = ['Aviation Specific', 'ToolBox','Maturity Level 1', 'Maturity Level 2','Maturity Level 3','Maturity Level 4','AI/ML Use'];
...
'Trendo Micro','Claroty','Nozomi Networks','Dragos','ForeScout Technologies','Boeing','AT&T','CISCO','Vectra','United Technologies'];
...
const handleMaturityLevel = (e) => {
  ...
  setFilterInput(prev => {
      ...
      const updatedLevels = computeMaturityLevels(currentLevels, level, checked);
...
const computeMaturityLevels = (currentLevels, newLevel, isChecked) => {
  let levels = Array.isArray(currentLevels) ? currentLevels : [currentLevels].filter(Boolean);
