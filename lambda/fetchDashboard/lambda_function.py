...
def prepare_scan_input(request_body: dict) -> dict:
    ...
    if filter_input:
        maturity_level = filter_input.pop("Maturity_Level", None)
        if maturity_level is not None:
            # Handle maturity_level as discrete values
            if isinstance(maturity_level, list):
                # Use .is_in() for lists to match any specified values
                maturity_condition = Attr("Maturity_Level").is_in(maturity_level)
            else:
                # Single integer value
                maturity_condition = Attr("Maturity_Level").eq(maturity_level)

            if filter_expression is None:
                filter_expression = maturity_condition
            else:
                filter_expression &= maturity_condition
    ...
    last_evaluated_key = last_evaluated_key_json if last_evaluated_key_json else None
...
def getDashboardToolData(scan_input: dict):
    ...
    for item in items:
        item["Customers"] = (
            list(item["Customers"])
            if isinstance(item.get("Customers"), set)
            else item.get("Customers")
        )
        item["Keywords"] = (
            list(item["Keywords"])
            if isinstance(item.get("Keywords"), set)
            else item.get("Keywords")
        )
...
def lambda_handler(event, context):
    ...
    response = {
        ...
        "body": json.dumps(
            {"tool_list": tool_list, "last_evaluated_key": last_evaluated_key}
        ),
    }
